class Student < ActiveRecord::Base
  ...
  scope :include_student_report_data,
    ...
    select("students.*, COUNT(courses.id) as course_count, SUM(courses.credits) AS course_credits,
           CONCAT(addresses.street, ', ', addresses.city, ', ', addresses.state) AS report_address").group("students.id")
  ...
  def course_count
    attributes.keys.include?("course_count") ? read_attribute(:course_count) : courses.count
  end
